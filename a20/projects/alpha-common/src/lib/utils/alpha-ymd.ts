/**
 * Utility class for parsing, formatting, and comparing dates in various formats.
 *
 * Features:
 * - Parse date strings in "yyyy-MM-dd" format to Date objects
 * - Format Date objects to strings in multiple formats (YMD, DMY, MDY, DM) and separators (Slash, Dash)
 * - Format date ranges
 * - Check if a date is within a range
 * - Compare and check equality of dates by year, month, and day
 * - Convert dates to normalized "YYYY-MM-DD" format
 *
 * All methods are pure and stateless except for instance methods, which operate on the provided date.
 */
export class AlphaYmd {
  ...
  static toYmd(date: Date): Date {
    let dt = this.parse(this.stringify(date));
    dt.setHours(0, 0, 0, 0);
    return dt;
  }
