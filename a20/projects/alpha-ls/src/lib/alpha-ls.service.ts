...
/**
 * AlphaLsService - Logging and Navigation Tracking Service
 *
 * Provides methods to post error logs and navigation logs to remote endpoints via HTTP POST requests.
 * Allows injection of custom logging delegates for error and navigation events.
 *
 * Usage:
 *   - Initialize with HttpClient and optional log URLs.
 *   - Use postErrorLog and postNavigationLog to send logs.
 *   - Optionally inject custom delegates for logging.
 *
 * Example:
 *   service.init(httpClient, errorLogUrl, navigationLogUrl);
