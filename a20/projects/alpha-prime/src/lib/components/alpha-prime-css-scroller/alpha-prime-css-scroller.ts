...
@Component({
  selector: 'alpha-prime-css-scroller',
  templateUrl: './alpha-prime-css-scroller.html',
  styleUrl: './alpha-prime-css-scroller.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class AlphaPrimeCssScroller<T = any> implements AfterViewInit {
  // Inputs
  ...
  addItems(newItems: T[]) {
    ...
    console.log(`CSS Scroller: Added ${newItems.length} items, total: ${this.allItems().length}, visible: ${this.visibleStartIndex()}-${this.visibleEndIndex()}`);
  }
  ...
  private checkScroll() {
    ...
    if (nearBottom && !this.loading() && !this.isNearBottom()) {
      ...
      console.log(`CSS Scroller: Near bottom, emitting loadMore (current: ${this.allItems().length})`);
